...
- (void)updateCount: (NSNumber *) newValue {
    self->count = [newValue intValue];
}
...
- (int)countBlackNodesIn:(RBNode<id>* _Nullable)currentPoint {
    if (!currentPoint) {
        ...
        return currentPoint.color == RB_BLACK ? total + 1 : total;
    }
}
...
- (bool)requiresColoring:(RBNode<id>*)currentNode {
    if (currentNode && currentNode.color == RB_RED && currentNode.parent && currentNode.parent.color == RB_RED) {
        RBNode<id>* uncle = [self getSiblingNode:currentNode.parent];
        return uncle != nil && uncle.color == RB_RED;
    } else {
        return false;
    }
}
...
- (bool)requiresRotation:(RBNode<id>*)currentNode {
    if (currentNode != nil && currentNode.color == RB_RED && currentNode.parent != nil && currentNode.parent.color == RB_RED) {
        RBNode<id>* uncle = [self getSiblingNode:currentNode.parent];
        return !uncle || uncle.color == RB_BLACK;
    }
    return false;
}
...
- (RBNode<id>* _Nullable)binaryDelete:(id _Nonnull)value OptionalPointerToNode:(RBNode<id>* _Nullable)node {
    ...
    int amountOfChildren = [node childrenCount];
...
- (BOOL)deleteValue:(id _Nonnull)value searchFrom:(RBNode<id>*)initialNode {
    ...
    if (nodeToDelete && [value isEqual:[nodeToDelete data]]) {
        int amountOfKids = [nodeToDelete childrenCount];
        if (amountOfKids == 2) {
            ...
            if (amountOfKids < 2 && nodeToDelete.color == RB_RED) {
                [self binaryDelete:value OptionalPointerToNode:nodeToDelete];
            } else if (nodeToDelete.color == RB_BLACK && amountOfKids == 1) {
                if (([nodeToDelete left] && nodeToDelete.left.color == RB_RED)
                    || ([nodeToDelete right] && nodeToDelete.right.color == RB_RED)) {
...
- (void)deleteCaseTwo:(RBNode<id>*)parentNode {
    RBNode<id>* redChild = [parentNode getRedChild];
    if (redChild && [redChild bothChildrenConsideredAsBlack]) {
...
- (void)deleteCaseThree:(RBNode<id>*)parentNode {
    if (parentNode.color == RB_BLACK) {
        ...
        if (nodeToExaminate.color == RB_BLACK && [nodeToExaminate bothChildrenConsideredAsBlack]) {
...
- (void)deleteCaseFour:(RBNode<id>*)parentNode {
    if (parentNode.color == RB_RED) {
        ...
        if ([self isBlack:nodeToExaminate] && nodeToExaminate != nil && [nodeToExaminate bothChildrenConsideredAsBlack]) {
            [parentNode swapColor];
            [nodeToExaminate swapColor];
            if ([parentNode childrenCount] == 2) {
...
- (void)deleteCaseFive:(RBNode<id>*)parentNode {
    if (parentNode.color == RB_BLACK) {
        ...
        if (nodeToExaminate.color == RB_BLACK && [nodeToExaminate getRedChild] != nil && ([nodeToExaminate getBlackChild] != nil)) {
            if (rightheight > leftheight && nodeToExaminate.left != nil && nodeToExaminate.left.color == RB_RED) {
                [self rotateRight:nodeToExaminate];
                [self changeColorAfterRightRotation:nodeToExaminate.parent];
            } else if (leftheight > rightheight && nodeToExaminate.right && nodeToExaminate.right.color == RB_RED) {
...
- (void)deleteCaseSix:(RBNode<id>*)parentNode {
    ...
    if (rightheight > leftheight && nodeToExaminate.right.color == RB_RED) {
        [self rotateLeft:parentNode];
        [self changeColorAfterLeftRotation:parentNode.parent];
        return;
    } else if (leftheight > rightheight && nodeToExaminate.left == RB_RED) {
        [self rotateRight:parentNode];
        [self changeColorAfterRightRotation:parentNode.parent];
        return;
    }
...
- (void)addObject:(id _Nonnull)value {
    ...
    if ([self requiresRotation:currentNode]) {
        if ([currentNode isLeftChild] && [currentNode.parent isLeftChild]) {
        ...
        } else if ([currentNode isRightChild] && [currentNode.parent isLeftChild]) {
            [self rotateLeft:currentNode.parent];
            [self rotateRight:currentNode.parent];
